...
def ingest_tree_cover_loss(self, state_fips=None):
    ...
    for county in counties:
        try:
            ...
            if not baseline:
                base_data = client.get_county_tree_cover_baseline(
            ...
            logger.error(
                f"Failed to ingest data for county {county.name} ({county.fips_code}): {e}"
            )

    logger.info(
        f"Successfully ingested tree cover loss for {success_count}/{counties.count()} counties."
    )
